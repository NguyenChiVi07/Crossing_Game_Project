...
PlayState::PlayState(GameManager *gameManager)
  ...
  m_pauseHUDBtn.setOrigin(
      {hudBtnBounds.size.x / 2.f, hudBtnBounds.size.y / 2.f});
  ...
  m_pauseTitleText.setOrigin(
      {titleBounds.size.x / 2.f, titleBounds.size.y / 2.f});
  m_pauseTitleText.setPosition({800.f, 160.f});

  auto setupPopupRow = [](sf::Sprite &sprite, sf::Text &text,
                          const std::string &str, float yPos) {
  ...
  auto setupTextButton = [](sf::Text &text, const std::string &str, float yPos,
...
void PlayState::Update(float delTime, sf::RenderWindow &window) {
  ...
  if (m_isShowingCollisionEffect) {
    ...
    return; // *** QUAN TRỌNG: Thoát Update tại đây để "đóng băng" mọi chuyển
            // động khác ***
  }
  ...
  if (m_overlayState == OverlayState::NONE) {
    ...
    checkCollision(); // Hàm này sẽ chuyển m_overlayState =
                      // OverlayState::GAME_OVER khi đụng vật cản
  ...
  if (m_overlayState == OverlayState::PAUSE_MENU) {
    auto applyHoverPopup = [&mousePos](sf::Sprite &sprite, sf::Text &text) {
      ...
      if (isHovered) {
        text.setFillColor(
            sf::Color(255, 215, 0)); // Chữ đổi sang màu vàng kim sáng
  ...
  if (isClicking && !m_isMousePressed) {
    ...
    // ----------------- TƯƠNG TÁC PAUSE MENU -----------------
      ...
      if (resumeBtn.getGlobalBounds().contains(mousePos) ||
          m_resumeLabel.getGlobalBounds().contains(mousePos)) {
        m_overlayState = OverlayState::NONE;
      } else if (loadBtn.getGlobalBounds().contains(mousePos) ||
                 m_loadLabel.getGlobalBounds().contains(mousePos)) {
      ...
      } else if (volumeBtn.getGlobalBounds().contains(mousePos) ||
                 m_volumeLabel.getGlobalBounds().contains(mousePos)) {
    ...
    // ----------------- TƯƠNG TÁC SETTINGS MENU -----------------
      ...
      if (m_setSfxBtn.getGlobalBounds().contains(mousePos)) {
        ...
        m_setMusicBtn.setString("MUSIC: " +
                                std::string(m_isMusicOn ? "ON" : "OFF"));
        ...
        m_overlayState = m_previousOverlayState; // Quay lại màn hình trước đó
                                                 // (Game Over hoặc Pause)
      }
        ...
        if (rand() % 5 == 0) {
          ...
          if (spawnObstacle) {
            ...
            Obstacle *newObstacle =
                new StaticObstacle(xPos + 50.f, yPos + 50.f);

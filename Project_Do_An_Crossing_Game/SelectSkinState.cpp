...
SelectSkinState::SelectSkinState(GameManager *gameManager)
    : mGameManager(gameManager),
      m_skinList({"Dude_Monster_Idle_4.png", "Owlet_Monster_Idle_4.png",
      ...
      m_enterText(AssetManager::getInstance().getFont("DIMIS___.ttf")),
    ...
    m_click(AssetManager::getInstance().getSoundBuffer("click.mp3"))
{
...

  {
    ...
    m_enterText.setOrigin({bounds.position.x + bounds.size.x / 2.f,
                           bounds.position.y + bounds.size.y / 2.f});
...

  {
    ...
    m_instructionText.setOrigin({bounds.position.x + bounds.size.x / 2.f,
                                 bounds.position.y + bounds.size.y / 2.f});
  ...
  for (const std::string &skin : m_skinList) {
    ...
    m_animations.push_back(
        std::make_unique<Animation>(&texture, sf::Vector2u(4, 1), 0.1f));
  }
...
void SelectSkinState::Render(sf::RenderWindow &window) 
{
  ...
  int leftIndex =
      (m_currentIndex - 1 + static_cast<int>(m_skinSprites.size())) %
      static_cast<int>(m_skinSprites.size());
